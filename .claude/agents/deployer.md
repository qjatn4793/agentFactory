...
runtime:
  ...
  rules: [general, mvp-generation, "backend/stack"]
---
...
빌드 검증을 통과한 MVP 산출물을 **로컬 Docker compose 환경** 으로 감싼다. 사용자는 `docker compose up --build -d` 한 번으로 mysql + backend + frontend 3 컨테이너를 띄우고 브라우저로 URL 을 열 수 있어야 한다.
...
- mysql, backend 는 호스트에 노출하지 않음. 컨테이너 간 내부 네트워크만 사용.
...
- mysql 데이터는 named volume `mysqldata_<jobId>` 로 영속화.
...
DB_ROOT_PASSWORD=<랜덤 16자 alphanum 생성>
ADMIN_USERNAME=admin
ADMIN_PASSWORD=<랜덤 16자 alphanum 생성>
```

`DB_ROOT_PASSWORD` 는 MySQL root 계정용(healthcheck 의 `mysqladmin ping -u root -p...` 에 사용). 일반 앱 연결은 `DB_USERNAME`/`DB_PASSWORD` 로만.
...
COPY application/api/build.gradle.kts application/api/
RUN gradle --no-daemon :application:api:dependencies || true
COPY domain ./domain
COPY infrastructure ./infrastructure
COPY application ./application
RUN gradle --no-daemon :application:api:bootJar -x test

FROM eclipse-temurin:21-jre
WORKDIR /app
COPY --from=build /src/application/api/build/libs/*.jar /app/app.jar
EXPOSE 8080
HEALTHCHECK --interval=10s --timeout=3s --start-period=40s --retries=5 \
  CMD wget -q -O- http://localhost:8080/v3/api-docs || exit 1
ENTRYPOINT ["java", "-jar", "/app/app.jar"]
```

- 멀티모듈 전제 — `COPY` 단계를 두 번에 나눠 build 스크립트만 먼저 가져와 의존성 캐시를 확보하고, 소스는 뒤에 복사해 incremental build 효율 확보.
- healthcheck 는 `/v3/api-docs` (springdoc). actuator 는 MVP 에 포함하지 않음.
- WebFlux 기반이라 `start-period` 를 40초로 상향 (R2DBC 초기화 + Flyway JDBC 마이그레이션).
...
services:
  mysql:
    ...
    healthcheck:
      test: ["CMD-SHELL", "mysqladmin ping -h localhost -u root -p$${DB_ROOT_PASSWORD} --silent"]
      interval: 5s
      timeout: 3s
      retries: 20
      start_period: 20s
...
volumes:
  mysqldata:
    name: agentfactory_${JOB_ID}_mysqldata
```

- `volumes.mysqldata.name` 을 명시해서 볼륨이 job 별로 분리된다 (`/reset-mvp` 가 안전하게 타겟팅).
- 네트워크는 기본 default 사용, 서비스명(`mysql`, `backend`) 으로 DNS 해석.
- healthcheck `$${DB_ROOT_PASSWORD}` 의 `$$` 는 compose 변수 보간을 이스케이프해 **컨테이너 내부**에서 치환되도록 한 것. compose 파싱 시 바로 치환되면 비밀번호가 ps 에 노출될 수 있다.
...
- `latest` 태그. 모든 이미지는 명시적 버전 (`mysql:8.0`, `nginx:1.27-alpine`, `node:22-alpine`, `gradle:8.10-jdk21`, `eclipse-temurin:21-jre`).
